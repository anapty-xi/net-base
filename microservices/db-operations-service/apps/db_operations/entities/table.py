...
def validate_title(title: str) -> str:
    ...
    if len(title) > 128:
        logger.critical(f'table_entity/ {title} превышает допустимую длину названия 128 < {len(title)}')
        raise ValueError(f'длина названия таблицы {len(title)} символа. максимально 128')
    return title

def validate_cols(cols: List[str]) -> List[str]:
    if len(cols) > 64:
        logger.critical(f'table_entity/ колличество стобцов ({len(cols)}) превышает 64')
    ...
    for index, col in enumerate(cols):
        if ' ' in col:
            ...
            cols[index] = col
        if len(col) > 128:
            logger.critical(f'table_entity/ длина названия столбца ({col, len(col)}) превышает 128')
    ...
    def validate_rows_number(self) -> Self:
        for row in self.rows:
            if len(row) != len(self.cols):
                logger.critical(f'table_entity/ количество столбцов - {len(self.cols)} не совпадает с количеством значений в строке - {len(row)}')
